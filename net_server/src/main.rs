...
use socket::tcp_connect_config::TcpConnectConfig;
use socket::tcp_connect_config::TcpConnectConfigBuilder;
use socket::tcp_connect_server::TcpConnectServer;
use std::sync::mpsc;
...
use net_server::Config;
use net_server::ConfigBuilder;
use net_server::Server;
...
fn main() {
    ...
    match Logger::init(&String::from("info"), &String::from("logs/net_server.log")) {
        Ok(()) => (),
...
fn net_thread_run(
    ...
    let mut tcp_connect_server: TcpConnectServer;
    match TcpConnectServer::new(&config, &mut net_msg_cb) {
        Ok(server) => tcp_connect_server = server,
        Err(err) => {
    ...
    loop {
        tcp_connect_server.tick();

        match tcp_connect_server.epoll_event(epoll_wait_timeout) {
        ...
        loop {
            ...
            match receiver.try_recv() {
                Ok(net_msg) => {
                    //这里要优化 判断是否广播消息，广告
                    tcp_connect_server.write_net_msg(net_msg);
