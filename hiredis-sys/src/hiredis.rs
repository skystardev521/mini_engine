...
impl RedisClient {
    ...
    /// timeout ms
    #[allow(dead_code)]
    fn redis_connect_timeout(ip: String, port: i32, timeout: u32) -> Result<RedisClient, String> {
        match CString::new(ip) {
            Ok(c_ip) => {
    ...
    #[allow(dead_code)]
    fn redis_connect_unix_timeout(path: String, timeout: u32) -> Result<RedisClient, String> {
        match CString::new(path) {
            Ok(c_path) => {
                let tv = Self::ms_to_tv(timeout as i64);
