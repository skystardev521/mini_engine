...
impl ThreadPool {
    pub fn new(size: u8) -> Self {
        ThreadPool {
            poll_idx: 0,
            threads: Vec::with_capacity(size as usize),
        }
    }
    ...
    */
    ...
    pub fn sender(&mut self, task_enum: TaskEnum) -> bool {
        ...
        loop {
            match self.threads[self.poll_idx].sender(mut_task) {
                Ok(()) => {
                    result = true;
                    self.next_poll_idx();
                    break;
...
impl ThreadsTrait<u64> for ThreadPool {
...
impl ThreadsTrait<QueryResult> for ThreadPool {
